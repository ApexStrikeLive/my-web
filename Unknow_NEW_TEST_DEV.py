...
API_KEY = 'lPh0cFLS7i8pmBksBIeNEsbH0QqCBEsKMFQtnCPy4IPbu1cU1su4iorf6OisOGW'
...
SIGNAL_LOG_FILE = 'Binance_OPT_DEV.json'
STRATEGY_NAME = "Binance_OPT_DEV"  # New: Strategy Name for JSON output

# --- Timeframes and Coins to Exclude ---
...
def check_recent_momentum(df):
    """Simple momentum check - last 3 candles should show direction"""
    if len(df) < 3:
        return True
    ...
    # For general momentum (works for both buy/sell)
    price_change = (closes[-1] - closes[0]) / closes[0] * 100
    return abs(price_change) > 0.5  # At least 0.5% move in 3 candles


# --- Logika wykrywania sygnałów ---

def check_signal(pair, data, timeframe, vwap, rsi, macd_data, volume_ratio):
    """
    ...
    if not check_recent_momentum(data):
        return None
...
def monitor_signals():
    ...
    while True:
        ...
        # Main loop to iterate through each timeframe
        for timeframe in TIMEFRAMES:
            print(f"\nScanning for signals on the {timeframe} timeframe...")

            with tqdm(total=total_pairs, desc=f"Analyzing pairs ({timeframe})", unit="pair", ncols=100) as pbar:
                for i, pair in enumerate(pairs_to_scan):
                    data = fetch_ohlcv(pair, timeframe, 100)
                    ...
                    # Pass the current timeframe to the signal check function
                    new_signal = check_signal(pair, data, timeframe, vwap, rsi, macd_data, volume_ratio)
                    if new_signal:
                        signal_key = (
